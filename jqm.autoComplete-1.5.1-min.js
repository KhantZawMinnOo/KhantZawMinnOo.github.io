...
(function (e) {
    ...
    r = function (t, n, r) {
        var s;
        if (r.builder) s = r.builder.apply(t.eq(0), [
            n,
            r
        ]);
         else {
            ...
            n && e.each(n, function (t, n) {
                e.isPlainObject(n) ? s.push('<li data-icon=' + r.icon + '><a href="' + r.link + encodeURIComponent(n.value) + '" data-transition="' + r.transition + '" data-autocomplete=\'' + JSON.stringify(n) + '\'>' + r.labelHTML(n.label) + '</a></li>')  : s.push('<li data-icon=' + r.icon + '><a href="' + r.link + encodeURIComponent(n) + '" data-transition="' + r.transition + '">' + r.labelHTML(n) + '</a></li>')
            })
        }
        e.isArray(s) && (s = s.join(''));
        e(r.target).html(s).listview('refresh');
        r.callback !== null && e.isFunction(r.callback) && i(r);
        if (s.length > 0) t.trigger('targetUpdated.autocomplete');
         else {
    ...
    o = function (t) {
        ...
        a,
        f,
        l = i.jqmData('autocomplete'),
        c,
        h;
        Date.now || (Date.now = function () {
            return (new Date).valueOf()
        });
        t && (t.keyCode === 38 ? e('.ui-btn-active', e(l.target)).removeClass('ui-btn-active').prevAll('li.ui-btn:eq(0)').addClass('ui-btn-active').length || e('.ui-btn:last', e(l.target)).addClass('ui-btn-active')  : t.keyCode === 40 ? e('.ui-btn-active', e(l.target)).removeClass('ui-btn-active').nextAll('li.ui-btn:eq(0)').addClass('ui-btn-active').length || e('.ui-btn:first', e(l.target)).addClass('ui-btn-active')  : t.keyCode === 13 && (e('.ui-btn-active a', e(l.target)).click().length || e('.ui-btn:first a', e(l.target)).click()));
        if (l) {
            a = i.val();
            if (l._lastText === a) return;
            if (l._retryTimeout) {
                window.clearTimeout(l._retryTimeout);
                l._retryTimeout = null
            }
            if (!(!t || t.keyCode !== 13 && t.keyCode !== 38 && t.keyCode !== 40)) return;
            if (a.length < l.minLength) s(i, e(l.target));
             else {
                ...
                if (e.isArray(l.source)) {
                    f = l.source.sort().filter(function (t) {
                        l.matchFromStart ? (c, h = new RegExp('^' + a, 'i'))  : (c, h = new RegExp(a, 'i'));
                        e.isPlainObject(t) ? c = t.label : c = t;
                        return h.test(c)
                    });
                    r(i, f, l)
                } else if (typeof l.source == 'function') l.source(a, function (e) {
                    r(i, e, l)
                });
                 else {
                    var p = {
                    ...
                    if (e.isPlainObject(l.source)) {
                        l.source.callback && l.source.callback(a, p);
                        for (var d in l.source) d !== 'callback' && (p[d] = l.source[d])
                    } else p.url = l.source;
                    l.termParam && (p.data[l.termParam] = a);
                    e.ajax(p)
                }
            }
        }
    ...
    u = {
        init: function (n) {
            var r = this;
            r.jqmData('autocomplete', e.extend({
            }, t, n));
